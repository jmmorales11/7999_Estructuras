...
void ListaDoble<T>::ordenamientoBurbuja(){
	...
	cout<<"---------------Objetos Ordenados correctamente-----------------" << endl;
	mostrarPorCabeza();
}
template <typename T>
void ListaDoble<T>::calcularValorMax(double pesoMax){
	double valorMax = 0.0;
	NodoDoble *aux=this->primero;
	while(pesoMax>0){
		if(pesoMax>aux->getObjeto()->getPeso()){
			pesoMax = pesoMax - aux->getObjeto()->getPeso();
			valorMax = valorMax + aux->getObjeto()->getValor();
		} else{
			valorMax = valorMax + ((aux->getObjeto()->getValor()*pesoMax)/aux->getObjeto()->getPeso());
			pesoMax = 0;
		}
		aux = aux->getSiguiente();
		if(aux==NULL){
			cout<<endl;
			cout<<"\nYa no existen mas objetos para llenar la mochila"<<endl;
			break;
		} 
	}
	cout<<"\nEl valor maximo de la mochila es: "<< valorMax<<endl;
}